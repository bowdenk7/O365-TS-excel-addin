...
export class FunnelChart {
    ...
    private _getLabel(ind: number): string {
        return <string>this.data[ind][0];
    };

    private _getEngagementCount(ind: number): number {
        return <number>this.data[ind][1];
    };

    private _createPaths(): any {
        /* Returns an array of points that can be passed into d3.svg.line to create a path for the funnel */
        let trapezoids: any = [];

        function findNextPoints(chart: any, prevLeftX: number, prevRightX: number, prevHeight: number, dataInd: number) {
            // reached end of funnel
            if (dataInd >= chart.data.length) return;

            // math to calculate coordinates of the next base
            let area = chart.data[dataInd][1] * chart._totalArea / chart.totalEngagement;
            let prevBaseLength = prevRightX - prevLeftX;
            let nextBaseLength = Math.sqrt((chart._slope * prevBaseLength * prevBaseLength - 4 * area) / chart._slope);
            let nextLeftX = (prevBaseLength - nextBaseLength) / 2 + prevLeftX;
            let nextRightX = prevRightX - (prevBaseLength - nextBaseLength) / 2;
            let nextHeight = chart._slope * (prevBaseLength - nextBaseLength) / 2 + prevHeight;

            let points = [[nextRightX, nextHeight]];
    ...
    public draw(elem: string, speed: number) {
        ...
        function drawTrapezoids(funnel: FunnelChart, i: number) {
            ...
            let nextHeight: number = paths[i][paths[i].length - 1];

            let node: any = trapezoid.node();
            let totalLength = node.getTotalLength();
